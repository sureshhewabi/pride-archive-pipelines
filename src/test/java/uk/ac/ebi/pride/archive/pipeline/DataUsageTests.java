...
public class DataUsageTests {
  ...
  private void validateReportOutputFile() throws IOException {
    ...
    Assert.assertTrue(
        "Report file should exist in output directory",
        reportDirFiles != null && reportDirFiles.length == 1);
    ReversedLinesFileReader reversedLinesFileReader =
        new ReversedLinesFileReader(reportDirFiles[0], Charset.forName("UTF-8"));
    ...
    Assert.assertTrue(
        "Report file is not blank and has a last line",
        0 < reportDirFiles[0].length() && !StringUtils.isEmpty(lastLine));
    ...
    Assert.assertTrue(
        "Final line should contain previous month YYYYMM, tab, final total byte count",
        finalMonth.format(yearMonthFormatter).equals(lineParts[0])
            && 0 < Long.parseLong(lineParts[1]));
  }
  ...
  private void setupTempDataDirectoriesAndFiles() throws Exception {
    ...
    File publicAccession =
        tempDatafolder.newFolder(publicYear.getName(), publicMonth.getName(), "PXT000001");
    File publicInternal =
        tempDatafolder.newFolder(
            publicYear.getName(), publicMonth.getName(), publicAccession.getName(), "internal");
    File publicSubmissionPx = new File(publicInternal.getPath() + File.separator + "submission.px");
    File publicSubmitted =
        tempDatafolder.newFolder(
            publicYear.getName(), publicMonth.getName(), publicAccession.getName(), "submitted");
    ...
    File privateSubmissionPx =
        new File(privateInternal.getPath() + File.separator + "submission.px");
    ...
    File privateSecondInternal =
        tempDatafolder.newFolder(privateSecondAccession.getName(), "internal");
    File privateSecondSubmissionPx =
        new File(privateSecondInternal.getPath() + File.separator + "submission.px");
    File privateSecondSubmitted =
        tempDatafolder.newFolder(privateSecondAccession.getName(), "submitted");
    File privateSecondSubmittedRaw =
        new File(privateSecondSubmitted.getPath() + File.separator + "bar.raw");
    File privateSecondSubmittedSearch =
        new File(privateSecondSubmitted.getPath() + File.separator + "foo.txt");
    ...
    File validatedSubmissionPx =
        new File(validatedInternal.getPath() + File.separator + "submission.px");
    File validatedSubmitted = tempDatafolder.newFolder(validatedAccession.getName(), "submitted");
    File validatedSubmittedRaw =
        new File(validatedSubmitted.getPath() + File.separator + "bar.raw");
    ...
    File resubPrevalidSubmissionPx =
        new File(resubPrevalidAccession.getPath() + File.separator + "submission.px");
    File resubPrevalidSubmittedRaw =
        new File(resubPrevalidAccession.getPath() + File.separator + "bar.raw");
   ...
   */
  private void createTempFileAndSetCreationTime(File tempFile, String localDateYearMonthDay)
      throws IOException {
    generateTempFile(tempFile);
    FileTime tempFiletime =
        FileTime.from(
            LocalDate.parse(localDateYearMonthDay).atStartOfDay().toInstant(ZoneOffset.UTC));
