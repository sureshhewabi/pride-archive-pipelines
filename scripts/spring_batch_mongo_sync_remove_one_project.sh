...
##### FUNCTIONS
printUsage() {
    echo "Description: In the revised archive pipeline, this will remove one assay information from mongoDB"
...
bsub -M ${MEMORY_LIMIT} \
     ...
     -u ${JOB_EMAIL} \
     -J ${JOB_NAME} \
     ./runPipelineInJava.sh ${LOG_PATH} ${LOG_FILE_NAME} ${MEMORY_LIMIT_JAVA}m -jar revised-archive-submission-pipeline.jar --spring.batch.job.names=resetMongoProjectsJob -Dspring-boot.run.arguments= --accession=${PROJECT_ACCESSION}
