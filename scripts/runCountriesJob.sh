...
LOG_PATH="/nfs/pride/work/archive/revised-archive-submission-scripts/log/pride-countries-data-${DATE}"
...
bsub -M 3000 -R "rusage[mem=3000]" -q production-rh7 -u ${EMAIL} -J ${JOB_NAME} -o ${LOG_PATH}_OUT -e ${LOG_PATH}_ERR "${JAVA_DIR}java -Xmx2500m -jar ${JAR_PATH} --spring.batch.job.names=PrideArchiveAnnotateProjectWithCountry"
