...
JOB_NAME="reanalysis-datasets-update"
# memory limit
MEMORY_LIMIT=6000
# memory overhead
...
bsub -M ${MEMORY_LIMIT} \
     ...
     -u ${JOB_EMAIL} \
     -J ${JOB_NAME} \
     ./runPipelineInJava.sh ${LOG_PATH} ${LOG_FILE_NAME} ${MEMORY_LIMIT_JAVA}m -jar revised-archive-submission-pipeline.jar --spring.batch.job.names=syncReanalysisDatasetsToMongoDBJob