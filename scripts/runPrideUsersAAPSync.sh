...
bsub -M 3000 -R "rusage[mem=3000]" -q production-rh74 -u ${EMAIL} -J ${JOB_NAME} -o ${LOG_PATH}_OUT -e ${LOG_PATH}_ERR "${JAVA_DIR}java -Xmx2500m -jar ${JAR_PATH} --spring.batch.job.names=PrideUsersAAPSync"
